...
func scanHospitalCity(row pgx.Row) (*HospitalCity, error) {
	...
	err := row.Scan(&c.ID, &c.Name, &c.Lat, &c.Lng, &c.RadiusM, &lastFetched, &c.Enabled)
	if err != nil {
		return nil, err
	}
...
const hospitalCityColumns = `id, name, lat, lng, radius_m, last_fetched, enabled`
...
func (s *HospitalCityStore) Create(ctx context.Context, c *HospitalCity) error {
	...
	const q = `INSERT INTO hospital_cities (id, name, lat, lng, radius_m, last_fetched, enabled) VALUES ($1,$2,$3,$4,$5,$6,$7)`
	_, err := s.pool.Exec(ctx, q, c.ID, c.Name, c.Lat, c.Lng, c.RadiusM, lastFetched, c.Enabled)
	return err
}
...
func (s *HospitalCityStore) Update(ctx context.Context, c *HospitalCity) error {
	...
	const q = `UPDATE hospital_cities SET name=$1, lat=$2, lng=$3, radius_m=$4, last_fetched=$5, enabled=$6 WHERE id=$7::uuid`
	_, err := s.pool.Exec(ctx, q, c.Name, c.Lat, c.Lng, c.RadiusM, lastFetched, c.Enabled, c.ID)
	return err
}
