...
// Config represents a referral type configuration stored in referral_configs table.
// Admins can configure reward amounts and ride thresholds per referral type.
type Config struct {
	ID             string  `db:"id" json:"_id,omitempty"`
	Type           string  `db:"type" json:"type"` // "user_to_user", "user_to_driver", "driver_to_user", "driver_to_driver"
	ReferrerAmount float64 `db:"referrer_amount" json:"referrer_amount"`
	NewUserAmount  float64 `db:"new_user_amount" json:"new_user_amount"`
	RidesRequired  int     `db:"rides_required" json:"rides_required"`
...
type Record struct {
	...
	Type             string     `db:"type" json:"type"` // "user_to_user", etc.
	ReferrerID       string     `db:"referrer_id" json:"referrer_id"`
	ReferrerRole     string     `db:"referrer_role" json:"referrer_role"` // "user" or "driver"
	RefereeID        string     `db:"referee_id" json:"referee_id"`
	RefereeRole      string     `db:"referee_role" json:"referee_role"` // "user" or "driver"
	Code             string     `db:"code" json:"code"`
	RidesRequired    int        `db:"rides_required" json:"rides_required"`
	RidesDone        int        `db:"rides_done" json:"rides_done"`
	ReferrerCredited bool       `db:"referrer_credited" json:"referrer_credited"`
	RefereeCredited  bool       `db:"referee_credited" json:"referee_credited"`
	ReferrerAmount   float64    `db:"referrer_amount" json:"referrer_amount"`
	RefereeAmount    float64    `db:"referee_amount" json:"referee_amount"`
